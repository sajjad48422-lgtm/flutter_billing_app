...
class HomePage extends StatefulWidget {
  /// مشخص می‌کند آیا تب فروش در حال حاضر تب فعال (قابل مشاهده) است یا خیر.
  /// چون این صفحه داخل IndexedStack قرار دارد و هیچوقت dispose نمی‌شود،
  /// باید از همین فلگ برای متوقف کردن دوربین وقتی کاربر به تب دیگری
  /// می‌رود استفاده کنیم؛ وگرنه دوربین در پس‌زمینه روشن می‌ماند و با
  /// دوربین صفحات دیگر (مثل اسکنر ثبت کالا) تداخل پیدا می‌کند.
  final bool isActive;
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  final MobileScannerController _scannerController = MobileScannerController(
    ...
    returnImage: false,
    autoStart: false, // دستی کنترل می‌کنیم تا با وضعیت _isCameraOn هماهنگ باشد
  );

  bool _isCameraOn = false;
  bool _isFlashOn = false;
  bool _wasCameraOnBeforeInactive = false;
  ...
  Future<void> _safeStart() async {
    try {
      ...
      // اگر دوربین در دسترس نبود (مثلاً مشغول صفحه دیگری بود) بی‌سروصدا رد شو
    }
  }
  ...
  Future<void> _safeStop() async {
    try {
      ...
      // کنترلر ممکن است از قبل متوقف باشد
    }
  }
  ...
  void didUpdateWidget(covariant HomePage oldWidget) {
    ...
    if (oldWidget.isActive && !widget.isActive) {
      // کاربر از تب فروش خارج شد: دوربین را کامل متوقف کن
      _wasCameraOnBeforeInactive = _isCameraOn;
      _safeStop();
    } else if (!oldWidget.isActive && widget.isActive) {
      // کاربر دوباره به تب فروش برگشت
      if (_wasCameraOnBeforeInactive && _isCameraOn) {
        _safeStart();
      }
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (!widget.isActive || !_isCameraOn) return;
    switch (state) {
              ...
              child: PrimaryButton(
                onPressed: state.cartItems.isEmpty
                    ...
                    : () async {
                        ...
                        if (_isCameraOn && widget.isActive && mounted) {
                          _safeStart();
                        }
                      },
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text(
              'دوربین را روشن کنید تا بارکدها به صورت خودکار اسکن شوند.',
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text(
              'کالاهای اسکن شده اینجا نمایش داده می‌شوند.',
              textAlign: TextAlign.center,
