...
class ReportsBloc extends Bloc<ReportsEvent, ReportsState> {
  ...
  Future<void> _onLoadDashboard(
      LoadDashboardEvent event, Emitter<ReportsState> emit) async {
    ...
    // یک کوئری برای ۸ روز گذشته (کافی برای امروز/دیروز/نمودار هفته)
    final result = await getSalesByDateRangeUseCase(yesterdayStart, now);
    ...
    result.fold(
      ...
      (sales) {
        ...
        for (final sale in sales) {
          if (!sale.createdAt.isBefore(todayStart)) {
            todayRevenue += sale.totalAmount;
            todayProfit += sale.totalProfit;
            todayOrderCount++;
          } else if (!sale.createdAt.isBefore(yesterdayStart)) {
            yesterdayRevenue += sale.totalAmount;
            yesterdayProfit += sale.totalProfit;
            yesterdayOrderCount++;
          }
        }
    ...
    // کوئری جدا برای نمودار ۷ روز گذشته (بازه‌ی بزرگ‌تر از امروز/دیروز)
    final weeklyResult = await getSalesByDateRangeUseCase(weekStart, now);
    weeklyResult.fold(
      (failure) => emit(state.copyWith(
        dashboardStatus: DashboardStatus.error,
        dashboardError: failure.message,
      ...
      (sales) {
        ...
        for (int i = 6; i >= 0; i--) {
          ...
          weeklyRevenue[ShamsiHelper.toShamsiShort(day)] = 0;
        }
        for (final sale in sales) {
          final key = ShamsiHelper.toShamsiShort(sale.createdAt);
          if (weeklyRevenue.containsKey(key)) {
            weeklyRevenue[key] = (weeklyRevenue[key] ?? 0) + sale.totalAmount;
          }
        }
        emit(state.copyWith(weeklyRevenue: weeklyRevenue));
      },
    );
  }
}
